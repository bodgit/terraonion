...
// Based on MAME's src/devices/bus/neogeo/prot_cmc.cpp
...
func cmc42GfxDecrypt(rom []byte, xor int) []byte {
	c := cmc{
		type0T03: [256]byte{
	...
	return c.gfxDecrypt(rom, xor)
}

func cmc50GfxDecrypt(rom []byte, xor int) []byte {
	c := cmc{
		type0T03: [256]byte{
...
func cmcSfixDecrypt(gfx []byte, size int) []byte {
	sfix := make([]byte, size)
	offset := len(gfx) - size
	for i := 0; i < size; i++ {
...
func m1AddressScramble(address int, key uint16) int {
	...
	p1 := [8][16]int{
		// Reversed compared to MAME source because I can just pass a slice then
...
func cmc50M1Decrypt(rom []byte) []byte {
	m1 := make([]byte, len(rom))

	key := uint16(0)
	for _, x := range rom[:0x10000] {
		key += uint16(x)
	}

	for i := 0; i < len(rom); i++ {
		if j := m1AddressScramble(i, key); j < len(rom) {
			m1[i] = rom[j]
		}
	}
